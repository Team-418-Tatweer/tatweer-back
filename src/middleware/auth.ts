...
import { MyRequest, UsersTypes } from '@myTypes/Express'
...
export const isHr = (req: MyRequest<UserD>, res: Response, next: NextFunction) => {
	const user = req.user as UserD;
	if (user.role === "hr") return next();
	ErrorResponse(res, HttpCodes.Unauthorized.code, authLogs.USER_ISN_T_HR.message, authLogs.USER_ISN_T_HR);
};

export const isManager = (req: MyRequest<UserD>, res: Response, next: NextFunction) => {
	const user = req.user as UserD;
	if (user.role === "manager") return next();
	ErrorResponse(res, HttpCodes.Unauthorized.code, authLogs.USER_ISN_T_MANAGER.message, authLogs.USER_ISN_T_MANAGER);
};
